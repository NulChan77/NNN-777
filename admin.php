    ...
    <?php while ($banner = $banner_result->fetch_assoc()): ?>
        <div>
            ...
            <form action="delete_banner.php" method="post" style="display:inline;">
                ...
                <button type="submit" class="delete-button">ลบแบนเนอร์</button>
            </form>
        </div>
    <?php endwhile; ?>
</div>
...
<form action="upload_banner.php" method="post" enctype="multipart/form-data">
    ...
    <button type="submit">อัปโหลดแบนเนอร์</button>
</form>
...
<div class="checkpoint-list">
    ...
    <form action="add_checkpoint.php" method="post" enctype="multipart/form-data">
        <div class="button-group">
            <a href="add_checkpoint.php" class="styled-button">เพิ่มจุดเช็คอิน</a>
            <a href="checkpoint_list.php" class="styled-button">ดูรายการจุดเช็คอิน</a>
        </div>
    </form>
</div>
...
<div class="checkpoint-list">
    ...
    <button class="styled-button" onclick="window.location.href='checkpoints.php'">รายละเอียด</button>
</div>
    ...
    <div class="carousel-slides">
        <div class="carousel-slide" style="background-color: #f0f0f0;">
            ...
            <form method="POST">
                ...
                <textarea name="text_content" required><?php echo isset($texts[1]) ? $texts[1] : ''; ?></textarea>
                <button type="submit" class="styled-button">บันทึก</button>
            </form>
        </div>
        <div class="carousel-slide" style="background-color: #e0e0e0;">
            ...
            <form method="POST">
                ...
                <textarea name="text_content" required><?php echo isset($texts[2]) ? $texts[2] : ''; ?></textarea>
                <button type="submit" class="styled-button">บันทึก</button>
            </form>
        </div>
        <div class="carousel-slide" style="background-color: #d0d0d0;">
            ...
            <form method="POST">
                ...
                <textarea name="text_content" required><?php echo isset($texts[3]) ? $texts[3] : ''; ?></textarea>
                <button type="submit" class="styled-button">บันทึก</button>
            </form>
        </div>
    </div>
...
<?php $conn->close(); ?>
        ...
        <form action="update_video.php" method="POST">
            <input type="text" name="video_url" placeholder="วางลิงก์ YouTube ที่นี่" 
                   value="<?php echo $row['video_url']; ?>" required>
...
<script>
    ...
    function deleteImage(imageId) {
        if (confirm("คุณต้องการลบรูปภาพนี้หรือไม่?")) {
            // ส่ง request ลบรูปไปยังเซิร์ฟเวอร์โดยใช้ fetch หรือ redirect ไปที่ลิงก์การลบ
            console.log("ลบรูปภาพ ID: " + imageId);
            // แนะนำให้ส่งคำขอลบไปยังเซิร์ฟเวอร์เพื่อดำเนินการตามขั้นตอนจริง
        }
    }
</script>
...
<script>
    $(document).ready(function() {
        // เมื่อกดบันทึกให้ทำงานฟังก์ชันนี้
        $('.tourist-item form').on('submit', function(e) {
            e.preventDefault(); // ป้องกันไม่ให้ฟอร์มส่งแบบธรรมดา
            var form = $(this);

            $.ajax({
                url: form.attr('action'), // URL ของไฟล์ PHP
...
<div class="footer-left">
    <a href="<?php echo $links['facebook'] ?? 'https://www.facebook.com'; ?>" target="_blank">
        <img src="uploads/icons8-facebook-50.png" alt="Facebook" title="Facebook">
    </a>
    <a href="<?php echo $links['youtube'] ?? 'https://www.youtube.com'; ?>" target="_blank">
        <img src="uploads/icons8-youtube-50.png" alt="YouTube" title="YouTube">
    </a>
    <a href="<?php echo $links['instagram'] ?? 'https://www.instagram.com'; ?>" target="_blank">
        <img src="uploads/instagram.png" alt="Instagram" title="Instagram">
    </a>
    <a href="<?php echo $links['twitter'] ?? 'https://www.twitter.com'; ?>" target="_blank">
        <img src="uploads/icons8-twitter-50.png" alt="Twitter" title="Twitter">
    </a>
</div>
...
$(document).ready(function() {
    // เมื่อฟอร์มถูกส่ง
    $('#social-links-form').on('submit', function(event) {
        ...
        // ส่งข้อมูลด้วย AJAX
        $.ajax({
            url: 'update_links.php', // URL ที่จะส่งข้อมูลไป
            type: 'POST',
            data: formData,
            success: function(response) {
                $('#response-message').html(response); // แสดงข้อความตอบกลับ
            },
            error: function() {
                $('#response-message').html('เกิดข้อผิดพลาดในการบันทึกข้อมูล');
            }
        });
    });
});
        ...
        <div class="footer-right">
            ...
            <form action="edit_contact.php" method="POST">
                ...
                <input type="text" name="opening_hours" value="<?php echo $row['opening_hours']; ?>" required>
    ...
    <script>
        ...
        const lines = [
            "ถ้าคุณกำลังมองหาสถานที่ท่องเที่ยวที่กำลังหา...",
            "เรามีจุดเช็คอินที่หลากหลายรอคุณอยู่!",
            "สนใจกดดูรายละเอียดเพิ่มเติมได้ทันที"
        ];
        let lineIndex = 0;
        let charIndex = 0;
